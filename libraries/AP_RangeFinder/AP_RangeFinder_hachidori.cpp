...
*/
AP_RangeFinder_HACHIDORI::AP_RangeFinder_HACHIDORI(RangeFinder::RangeFinder_State &_state, AP_RangeFinder_Params &_params) :
...
void AP_RangeFinder_HACHIDORI::update(void)
{
    WITH_SEMAPHORE(_sem);
    if (_new_distance) {
        state.distance_m = _distance * 0.01f;
        _new_distance = false;
        update_status();
    } else {
...
#endif // AP_RANGEFINDER_HACHIDORI_ENABLED
