...
bool AP_InertialSensor_HACHIDORI::_init(void)
{
    ...
    if (!_imu.register_gyro(_gyro_instance, 1000, 1) ||
        !_imu.register_accel(_accel_instance, 1000, 1)) {
        return false;
    }
...
void AP_InertialSensor_HACHIDORI::_timer_update(void)
{
    while (true) {
        ...
        if (pkt.tos == HACHIDORI_TOS_IMU) {
            ...
            union { float f; uint32_t u32; } x, y, z, t;
            // accel
            x.u32 = le32toh(fp[0]);
            y.u32 = le32toh(fp[1]);
            z.u32 = le32toh(fp[2]);
            ...
            // temp
            t.u32 = le32toh(fp[6]);
            _temp_filtered = _temp_filter.apply(t.f);
        }
    }
}
...
bool HACHIDORI_PORT::check_port(struct HACHIDORI_packet &pkt, bool take)
{
    ...
    static struct HACHIDORI_packet pktbuf[HACHIDORI_MAX_BUNDLE];
    static int npkt = 0;
    static int idx = 0;
    ...
    if (npkt == 0) {
        ...
        int n = recvfrom(fd, &pktbuf, sizeof(pktbuf), 0,
                         (struct sockaddr *) &caddr, &clilen);
    ...
    memcpy(&pkt, &pktbuf[idx], sizeof(pkt));
    ++idx;
    --npkt;
